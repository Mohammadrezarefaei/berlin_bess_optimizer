...
st.set_page_config(page_title="BESS Dispatch & Market Optimization", layout="wide")
...
net_power = np.where((hours >= 0) & (hours <= 2), 0.9, np.where((hours >= 18) & (hours <= 21), -0.8, 0.0))
fcr_headroom = np.where((hours >= 0) & (hours <= 2), 0.3, np.where((hours >= 19), 0.3, 0.0))
